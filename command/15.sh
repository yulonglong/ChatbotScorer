...
do
    ...
    -tr data/${dataset}.xml \
    -o expt${expt_num}${gpu_num}-${rand}-d${dataset}-v${vocab_size}-e${embedding_size}${embedding}-t${model_type}-p${pooling_type}-c${cnn_dim}w${cnn_win}cl${cnn_layer}-r${rnn_type}${rnn_dim}rl${rnn_layer}-a${optimizer}-b${batch_size}-seed${rand}${gpu_num}78-${gpu_name} \
    -lt ${label_type} \
